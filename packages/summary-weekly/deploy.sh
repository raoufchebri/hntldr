...
# Configuration
STACK_NAME="hacker-news-weekly-summarizer-${ENV}"
FUNCTION_NAME="hacker-news-weekly-summarizer-${ENV}"
REGION="us-east-1"
DEPLOY_BUCKET_NAME="hn-tracker-lambda-1742836337"
FUNCTION_ZIP="weekly-function.zip"
...
if [ -z "$ELEVEN_LABS_API_KEY" ]; then
  echo "Error: ELEVEN_LABS_API_KEY environment variable is not set"
  exit 1
fi
...
echo "Your weekly summarizer function is now scheduled to run every Friday at 08:00 UTC." 